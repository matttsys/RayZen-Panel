...
## Subscription profiles and per-profile request limits

`GET panel/platform/links` → `{ "profiles": [...], "max": …, "maxRequestLimit": … }`. Each
profile carries `token`, `name`, `createdAt`, `expiresAt`, `enabled`, `requests`,
`requestLimit`, `remaining`, `lastSeenAt`, `lastSeenFrom` and a derived `status` of
`active`, `disabled`, `expired` or `exhausted`. Both ceilings travel with the list so the
caller validates against the module's bound instead of a copy that drifts.

`POST panel/platform/links/create` with `{ "name", "days", "requestLimit"? }`. The token is
generated server-side and never accepted from the request. `requestLimit` is omitted or
`null` for an unlimited profile; `0` is not a sentinel for "unlimited".

`POST panel/platform/links/update` with `{ "token", "action", "requestLimit"? }`, where
`action` is one of:

| Action | Effect |
| --- | --- |
| `enable` / `disable` | Flips `enabled`, keeping the row and its counters |
| `delete` | Removes the profile |
| `limit` | Sets `requestLimit`; an explicit `null` removes the limit |
| `reset-count` | Returns `requests` to zero, which is what makes a limit renewable |

`requestLimit` is read only for `limit`, and a value that is neither `null` nor a positive
integer up to `maxRequestLimit` is a 400 that writes nothing. The response is
`{ "token", "action", "remaining", "profile" }`, where `profile` is the Panel's own copy
after the write — `null` only for `delete` — so the Companion never applies a limit change
optimistically.
