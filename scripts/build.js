...
async function processHtmlPages() {
    ...
    for (const dir of pageDirectories()) {
        ...
        if (dir !== 'error') {
            let css = readFileSync(base('style.css'), 'utf8');
            if (dir === 'panel' && css.includes('/* VAZIRMATN_FONT_FACE */')) {
                const fontFile = join(ASSET_PATH, 'fonts', 'Vazirmatn.woff2');
                const fontFace = existsSync(fontFile)
                    ? `@font-face{font-family:'Vazirmatn';src:url(data:font/woff2;base64,${readFileSync(fontFile).toString('base64')}) format('woff2');font-style:normal;font-weight:100 900;font-display:swap}`
                    : '';
                css = css.replace('/* VAZIRMATN_FONT_FACE */', fontFace);
            }
