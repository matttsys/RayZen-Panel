...
describe('the header set itself', () => {
    ...
    it('permits connect only to the origins each page actually uses', () => {
        ...
        // Same-origin only: the "My IP" card reads the visitor's address from the
        // request's own Cloudflare properties, and the version check is `./panel/version`.
        expect(panel).toMatch(/connect-src 'self'\s*;/);
