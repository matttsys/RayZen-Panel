...
import { MAX_PROFILES, MAX_REQUEST_LIMIT } from '@features/profiles';
...
describe('scanner routes', () => {
    ...
    it('applies a selected clean IP without replacing unrelated settings', async () => {
        ...
        const response = await call(kv, 'scanner/apply', {
            cookie: await session(),
            method: 'POST',
            body: { address: '104.16.0.1' }
        });
        const { body: result } = await body<{ accepted: number; cleanIPs: string[]; changed: boolean; rejected: unknown[] }>(response);

        expect(response.status).toBe(200);
        expect(result.accepted).toBe(1);
        expect(result.rejected).toEqual([]);
        expect(result.cleanIPs).toEqual(['104.16.0.1']);
        expect(result.changed).toBe(true);

        const stored = JSON.parse(kv.store.get('proxySettings') ?? '{}');
        expect(stored.cleanIPs).toEqual(['104.16.0.1']);
        expect(stored.remoteDNS).toBe('https://8.8.8.8/dns-query');
        expect(stored.ports).toEqual([443]);
    });

    it('accepts the valid part of a mixed apply and reports each rejection with a reason', async () => {
        const kv = deployment();
        const response = await call(kv, 'scanner/apply', {
            cookie: await session(),
            method: 'POST',
            body: { addresses: ['104.16.0.1', '104.16.0.1', '8.8.8.8', 'not-an-ip'], mode: 'replace' }
        });
        const { body: result } = await body<{
            accepted: number;
            cleanIPs: string[];
        ...
        expect(result.cleanIPs).toEqual(['104.16.0.1']);
        expect(result.rejected).toEqual([
            { address: '104.16.0.1', reason: 'duplicate' },
            { address: '8.8.8.8', reason: 'outside-published-cloudflare-ranges' },
            { address: 'not-an-ip', reason: 'not-an-ipv4-address' }
        ]);
    });

    it('applies an arbitrary-length address array, so there is no per-call item cap', async () => {
        const kv = deployment();
        const addresses = Array.from({ length: 300 }, (_, index) => `104.16.${Math.floor(index / 256)}.${index % 256}`);

        const response = await call(kv, 'scanner/apply', {
            cookie: await session(),
            method: 'POST',
            body: { addresses, mode: 'replace' }
        });
        const { body: result } = await body<{ accepted: number; cleanIPs: string[] }>(response);

        expect(response.status).toBe(200);
        expect(result.accepted).toBe(300);
        expect(result.cleanIPs).toHaveLength(300);
    });
    ...
    it('can append a clean IP and reports an idempotent apply', async () => {
        ...
        const appended = await body<{ cleanIPs: string[]; changed: boolean }>(
            await call(kv, 'scanner/apply', {
                cookie,
                method: 'POST',
                body: { address: '172.64.0.5', mode: 'append' }
            })
        );
        expect(appended.body.cleanIPs).toEqual(['104.16.0.1', '172.64.0.5']);
        expect(appended.body.changed).toBe(true);
        ...
        expect(repeated.body.cleanIPs).toEqual(['104.16.0.1', '172.64.0.5']);
    ...
    it('caps the stored clean-IP list, which is a settings-size bound rather than a scanner bound', async () => {
        const existing = Array.from({ length: 1000 }, (_, index) => `104.16.${Math.floor(index / 256)}.${index % 256}`);
    ...
    it('can apply multiple clean IP addresses in one request', async () => {
        const kv = deployment();
        const cookie = await session();
        ...
        const response = await call(kv, 'scanner/apply', {
            cookie,
            method: 'POST',
            body: { addresses: ['104.16.0.1', '172.64.0.5'], mode: 'replace' }
        });
        const { body: result } = await body<{ accepted: number; cleanIPs: string[]; changed: boolean }>(response);

        expect(response.status).toBe(200);
        expect(result.accepted).toBe(2);
        expect(result.cleanIPs).toEqual(['104.16.0.1', '172.64.0.5']);
        expect(result.changed).toBe(true);

        const stored = JSON.parse(kv.store.get('proxySettings') ?? '{}');
        expect(stored.cleanIPs).toEqual(['104.16.0.1', '172.64.0.5']);
    });

    it('clean-ips route supports GET and POST bulk update', async () => {
        const kv = deployment();
        const cookie = await session();

        const getRes = await call(kv, 'clean-ips', { cookie });
        const { body: initial } = await body<{ cleanIPs: string[]; max: number }>(getRes);
        expect(getRes.status).toBe(200);
        expect(initial.max).toBe(1000);
        expect(initial.cleanIPs).toEqual([]);

        const postRes = await call(kv, 'clean-ips', {
            cookie,
            method: 'POST',
            body: { addresses: ['1.1.1.1', '8.8.8.8'], mode: 'replace' }
        });
        const { body: updated } = await body<{ cleanIPs: string[]; changed: boolean }>(postRes);
        expect(postRes.status).toBe(200);
        expect(updated.cleanIPs).toEqual(['1.1.1.1', '8.8.8.8']);
        expect(updated.changed).toBe(true);

        const verifyGet = await body<{ cleanIPs: string[] }>(await call(kv, 'clean-ips', { cookie }));
        expect(verifyGet.body.cleanIPs).toEqual(['1.1.1.1', '8.8.8.8']);
    });

    it('clean-ips PUT stores the list verbatim, so a reorder is one save', async () => {
        const kv = deployment();
        const cookie = await session();

        await call(kv, 'clean-ips', { cookie, method: 'POST', body: { addresses: ['a.example', 'b.example', 'c.example'] } });

        const reordered = await body<{ cleanIPs: string[] }>(
            await call(kv, 'clean-ips', { cookie, method: 'PUT', body: { cleanIPs: ['c.example', 'a.example', 'b.example'] } })
        );
        expect(reordered.body.cleanIPs).toEqual(['c.example', 'a.example', 'b.example']);

        const afterDelete = await body<{ cleanIPs: string[] }>(
            await call(kv, 'clean-ips', { cookie, method: 'DELETE', body: { addresses: ['a.example'] } })
        );
        expect(afterDelete.body.cleanIPs).toEqual(['c.example', 'b.example']);
    });

    it('modes reads the proxy and fragment selections and rejects an unknown value', async () => {
        const kv = deployment();
        const cookie = await session();

        const read = await body<{ modes: Array<{ field: string; value: string; options: Array<{ id: string }> }> }>(
            await call(kv, 'modes', { cookie })
        );
        expect(read.body.modes.map(mode => mode.field)).toEqual(['proxyIpMode', 'fragmentMode']);
        expect(read.body.modes[0].options.map(option => option.id)).toEqual(['proxyip', 'prefix']);

        const rejected = await call(kv, 'modes', { cookie, method: 'POST', body: { proxyIpMode: 'nonsense' } });
        expect(rejected.status).toBe(400);

        // `proxyIpMode` is part of the deployment identity, not of `proxySettings`.
        const applied = await call(kv, 'modes', { cookie, method: 'POST', body: { proxyIpMode: 'prefix' } });
        expect(applied.status).toBe(200);
        expect(JSON.parse(kv.store.get('rz:identity') ?? '{}').proxyIpMode).toBe('prefix');

        const fragment = await call(kv, 'modes', { cookie, method: 'POST', body: { fragmentMode: 'high' } });
        expect(fragment.status).toBe(200);
        expect(JSON.parse(kv.store.get('proxySettings') ?? '{}').fragmentMode).toBe('high');
    });

    it('scanner/candidates accepts a Cloudflare CIDR and refuses one outside the published list', async () => {
        const kv = deployment();
        const cookie = await session();

        const inside = await body<{ range: string; candidates: string[] }>(
            await call(kv, 'scanner/candidates', { cookie, query: 'kind=clean-ip&count=8&range=104.16.0.0/16' })
        );
        expect(inside.body.range).toBe('104.16.0.0/16');
        expect(inside.body.candidates).toHaveLength(8);
        expect(inside.body.candidates.every(address => address.startsWith('104.16.'))).toBe(true);

        const outside = await call(kv, 'scanner/candidates', { cookie, query: 'kind=clean-ip&count=8&range=8.8.8.0/24' });
        expect(outside.status).toBe(400);
    });

    it('subscriptions/urls returns usable subscription URLs for all clients and formats', async () => {
        const kv = deployment();
        const cookie = await session();

        const response = await call(kv, 'subscriptions/urls', { cookie });
        const { body: result } = await body<{ securePath: string; urls: Array<{ type: string; core: string; url: string; path: string }> }>(response);

        expect(response.status).toBe(200);
        expect(result.securePath).toBe(TEST_SECURE_PATH);
        expect(result.urls.length).toBeGreaterThan(0);

        const normalXray = result.urls.find(u => u.type === 'normal' && u.core === 'xray');
        expect(normalXray).toBeDefined();
        expect(normalXray?.path).toContain(`/${TEST_SECURE_PATH}/sub/normal?app=xray`);

        const singBox = result.urls.find(u => u.core === 'sing-box');
        expect(singBox).toBeDefined();
        expect(singBox).toHaveProperty('importUrl');
    });

    it('presets/apply applies preset and updates settings', async () => {
        const kv = deployment();
        const cookie = await session();

        const response = await call(kv, 'presets/apply', {
            cookie,
            method: 'POST',
            body: { id: 'smart-low-latency' }
        });
        const { body: result } = await body<{ preset: { id: string }; changed: string[] }>(response);

        expect(response.status).toBe(200);
        expect(result.preset.id).toBe('smart-low-latency');
        expect(result.changed.length).toBeGreaterThan(0);

        const stored = JSON.parse(kv.store.get('proxySettings') ?? '{}');
        expect(stored.enableTFO).toBe(true);
        expect(stored.fragmentMode).toBe('low');
    });
});
...
describe('subscription link management', () => {
    ...
    it('link routes need a session like every other platform route', async () => {
        ...
        expect(await list(deployment(), await session())).toEqual({
            profiles: [],
            max: MAX_PROFILES,
