...
describe('validateSettings aggregate', () => {
    it('rejects a null form', () => {
        expect(hasError(validateSettings(null) ?? [], 'Settings')).toBe(true);
    });
    ...
    it('rejects a non-object body rather than throwing', () => {
        expect(hasError(validateSettings('not an object' as unknown as PanelSettings) ?? [], 'Settings')).toBe(true);
        expect(hasError(validateSettings(42 as unknown as PanelSettings) ?? [], 'Settings')).toBe(true);
        expect(hasError(validateSettings([] as unknown as PanelSettings) ?? [], 'Settings')).toBe(true);
    });
});
