...
describe('the theme axis reaches every theme', () => {
    it.each(THEMES)('%s defines its own accent in both modes', theme => {
        // Ocean also appears on bare `:root` for first paint, but every theme keeps
        // an explicit selector so switching themes never relies on source order.
        const selector = String.raw`:root\[data-theme="${theme}"\]`;
        const light = new RegExp(`${selector}\\s*\\{[^}]*--rz-p-accent:`, 'u');
