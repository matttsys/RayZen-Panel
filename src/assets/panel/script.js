...
async function checkVersion(panelVersion) {
    try {
        // RayZen does not poll a third-party (legacy upstream) upstream for releases. Point this at
        // the RayZen release feed when one exists; until then the check fails safe so the
        // panel never surfaces an unrelated project's version.
