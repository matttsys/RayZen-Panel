...
const RZ_GLYPH_PATHS = {
    ...
    keyboard_arrow_up: '<path d="M45 28 27 47Q26 48 26 49Q26 51 27 52Q28 53 30 53Q31 53 32 52L48 37L64 52Q65 53 66 53Q68 53 69 52Q70 51 70 49Q70 48 69 47L51 28Q50 28 50 27Q49 27 48 27Q47 27 46 27Q46 28 45 28Z"/>',
...
/**
 * One inline sprite, defined once. Every icon is a `<symbol>` here and every use site
 * is a `<use>`, so the page carries each outline exactly once and an icon is sized by
 * the font size of the element it sits in rather than by a hard-coded pixel box.
 */
function rzEnsureIconSprite() {
    if (document.getElementById('rz-icon-sprite')) return;
    const symbols = [
        ...Object.entries(RZ_ICON_PATHS).map(([key, body]) => `<symbol id="rz-i-${key}" ${RZ_ICON_ATTRS}>${body}</symbol>`),
        ...Object.entries(RZ_GLYPH_PATHS).map(([key, body]) => `<symbol id="rz-i-${key}" ${RZ_GLYPH_ATTRS}>${body}</symbol>`)
    ];
    const sprite = document.createElement('div');
    sprite.innerHTML = `<svg id="rz-icon-sprite" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" style="position:absolute;width:0;height:0;overflow:hidden">${symbols.join('')}</svg>`;
    (document.body || document.documentElement).prepend(sprite.firstElementChild);
}
...
function rzIconMarkup(name) {
    ...
    // vocabulary.
    const resolved = key && RZ_ICON_PATHS[key] ? key : RZ_GLYPH_PATHS[name] ? name : null;
    if (!resolved) return null;
    rzEnsureIconSprite();
    return `<svg class="rz-svg" aria-hidden="true" focusable="false"><use href="#rz-i-${resolved}"></use></svg>`;
}
...
// Hero mark: a geometric cat outline on the icon grid's 1.6 stroke. Decorative only.
const RZ_CAT_MARK = '<svg viewBox="0 0 64 64" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M33 14.5 31 6l7.5 5.5M47 14.5 49 6l-7.5 5.5"/><circle cx="40" cy="21" r="9"/><path d="M14 52c0-14 8-21 18-22M50 30c6 6 6 16 2 22M12 52h42M12 52c-8 0-8-12 0-12"/><path d="M36.5 20.5h.01M43.5 20.5h.01" stroke-width="2.2"/></svg>';
...
function installRayZenShell() {
    ...
    const hero = elm('section', { className: 'rz-hero rz-hero-unknown' }, [
        ...
        elm('span', { className: 'rz-hero-mark', 'aria-hidden': 'true', innerHTML: RZ_CAT_MARK }),
...
async function rayzenApi(route, options) {
    ...
    if (!payload) throw new Error(`Request failed (HTTP ${response.status}): the server did not answer with JSON.`);
    if (!response.ok || !payload.success) throw new Error(`${payload.message || 'Request failed'} (HTTP ${response.status})`);
    return payload.body;
}
...
function renderOverview(center, preflight, recs, scans, metrics, links) {
    ...
    setRzContent('rz-overview-content', [important, elm('section', { className: 'rz-command-grid' }, [active, recommendationPanel]), footer, setupSteps(preflight)]);
}
...
function renderAnalytics(metrics, history, effectiveness) {
    ...
    const days = Array.isArray(metrics?.snapshot?.days) ? metrics.snapshot.days : [];
    if (!days.length && !entries.length) {
        return setRzContent('rz-analytics-content', [elm('div', { className: 'rz-card rz-empty', role: 'status' }, [
            rzIcon('analytics'),
            elm('h3', {}, t('No traffic recorded yet')),
            elm('p', {}, t('Daily aggregate counters appear here once the panel has been used. Nothing per-user or per-request is stored.'))
        ])]);
    }
...
function setupSteps(preflight) {
    const steps = elm('div', { className: 'rz-onboarding-grid' }, [
        setupStep('1', 'Configure once', 'Identity, DNS, and routing.'),
        setupStep('2', 'Import one subscription', 'Import RayZen into your preferred client.'),
        setupStep('3', 'Measure endpoints', 'Score endpoint quality and stability.'),
        setupStep('4', 'Stay in control', 'Review every recommendation before applying it.')
    ]);
    const details = elm('details', { className: 'rz-setup-steps' }, [
        elm('summary', {}, preflight?.ready === false ? 'Setup steps — setup is not finished' : 'Setup steps'),
        steps
    ]);
    // Open by default only while setup is unfinished, so the panel explains itself once
    // and then gets out of the way. It used to be a dialog on every load, which is worse.
    if (preflight?.ready === false) details.open = true;
    return details;
}

function setupStep(number, title, copy) {
...
Object.assign(RZ_FA, {
 ...
 'Not available':'در دسترس نیست','Requests served in the last day, from your Cloudflare account. Needs an account token; otherwise this stays unavailable.':'درخواست‌های پاسخ‌داده‌شده در روز گذشته، از حساب Cloudflare شما. به توکن حساب نیاز دارد؛ در غیر این صورت در دسترس نخواهد بود.','The address this browser reaches the panel from, as Cloudflare sees it. Nothing is sent to a third-party lookup service.':'آدرسی که این مرورگر از آن به پنل می‌رسد، همان‌طور که Cloudflare می‌بیند. هیچ داده‌ای به سرویس جست‌وجوی شخص ثالث ارسال نمی‌شود.','Connect a bot so the panel can be read and managed from Telegram. Both fields are required.':'یک بات متصل کنید تا پنل از تلگرام خوانده و مدیریت شود. هر دو فیلد لازم است.','A resolver address served by this deployment. Paste it into a client that supports DoH.':'آدرس resolver که این استقرار ارائه می‌دهد. آن را در کلاینتی که DoH را پشتیبانی می‌کند وارد کنید.','Which apps open each subscription format, by platform. Pick one before copying a link.':'کدام برنامه‌ها هر قالب اشتراک را باز می‌کنند، به تفکیک پلتفرم. پیش از کپی لینک یکی را انتخاب کنید.','Not reported':'گزارش نشده','No traffic recorded yet':'هنوز ترافیکی ثبت نشده','Daily aggregate counters appear here once the panel has been used. Nothing per-user or per-request is stored.':'شمارنده‌های تجمعی روزانه پس از استفاده از پنل اینجا نمایش داده می‌شوند. هیچ داده‌ای به‌ازای کاربر یا درخواست ذخیره نمی‌شود.',
 ...
 'My IP':'آی‌پی من','Information':'اطلاعات','Value':'مقدار','Country':'کشور','City':'شهر','ISP':'سرویس‌دهندهٔ اینترنت',
...
async function getUsage() {
    try {
        ...
        if (available === false) {
            totalReq.textContent = t('Not available');
            panelReq.textContent = t('Not available');
            document.querySelectorAll('.usage-pct').forEach(node => { node.hidden = true; });
            return;
        }
        document.querySelectorAll('.usage-pct').forEach(node => { node.hidden = false; });
...
function renderPanel(proxySettings, tgSettings, subscriptions, clients) {
    ...
    textareaElements.forEach(elm => {
        ...
        // Same reason as the select above: a settings document written by an older build
        // simply has no entry for a textarea added since, and reading `.length` off that
        // threw before the panel finished its first render.
    ...
    renderCleanIPList(proxySettings.cleanIPs || []);
    initCleanIPManager();
...
async function fetchIPInfo() {
    ...
    const set = (id, value) => { const node = document.getElementById(id); if (node) node.textContent = value || t('Not reported'); };
    const details = await getIpDetails('');
    const code = details?.countryCode;
    const flag = code && /^[A-Z]{2}$/.test(code) ? String.fromCodePoint(...[...code].map(c => 0x1F1E6 + c.charCodeAt(0) - 65)) + ' ' : '';
    set('ip', details?.ip);
    set('country', details?.country ? `${flag}${details.country}` : '');
...
let rzCleanIPManagerInitialized = false;

function isValidCleanIPHost(value) {
    ...
    if (str.startsWith('[') && str.endsWith(']')) {
        ...
        return ipv6.test(inner) && inner.includes(':');
    }
    if (str.includes(':')) {
        const ipv6 = /^[0-9a-fA-F:]+$/;
    ...
    if (ipv4.test(str)) return true;
    const domain = /^(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,63}$/;
    return domain.test(str);
}

function getCleanIPsList() {
    const field = document.getElementById('cleanIPs');
    if (!field) return [];
    return (field.value || '').split(/[\r\n,]+/u).map(v => v.trim()).filter(Boolean);
}

function setCleanIPsList(ips) {
    const field = document.getElementById('cleanIPs');
    if (!field) return;
    field.value = ips.join('\r\n');
    renderCleanIPList(ips);
    handleProxyFormChanges();
}

function renderCleanIPList(ips) {
    const list = document.getElementById('cleanIPList');
    const badge = document.getElementById('cleanIPBadge');
    if (badge) badge.textContent = `${ips.length}/40`;
    if (!list) return;
    list.replaceChildren();

    if (!ips || ips.length === 0) {
        list.appendChild(elm('div', { className: 'rz-clean-ip-empty', textContent: t('No clean IPs configured. Default hostname will be used.') }));
        return;
    }

    ips.forEach((ip, index) => {
        const row = elm('div', { className: 'rz-clean-ip-item' }, [
            elm('span', { className: 'rz-clean-ip-num', textContent: String(index + 1) }),
            elm('span', { className: 'rz-clean-ip-host', textContent: ip, title: ip }),
            elm('div', { className: 'rz-clean-ip-actions' }, [
                elm('button', {
                    type: 'button',
                    className: 'rz-clean-ip-btn rz-clean-ip-up',
                    title: t('Move up'),
                    'aria-label': `${t('Move up')}: ${ip}`,
                    disabled: index === 0,
                    onclick: () => {
                        if (index === 0) return;
                        const next = [...ips];
                        ...
                        next[index] = temp;
                        setCleanIPsList(next);
                    }
                }, createIcon('keyboard_arrow_up')),
                elm('button', {
                    type: 'button',
                    className: 'rz-clean-ip-btn rz-clean-ip-down',
                    title: t('Move down'),
                    'aria-label': `${t('Move down')}: ${ip}`,
                    disabled: index === ips.length - 1,
                    onclick: () => {
                        if (index === ips.length - 1) return;
                        const next = [...ips];
                        const temp = next[index + 1];
                        next[index + 1] = next[index];
                        next[index] = temp;
                ...
                }, createIcon('keyboard_arrow_down')),
                elm('button', {
                    type: 'button',
                    className: 'rz-clean-ip-btn rz-clean-ip-delete',
                    title: t('Delete'),
                    'aria-label': `${t('Delete')}: ${ip}`,
                    onclick: () => {
                        const next = ips.filter((_, i) => i !== index);
                        setCleanIPsList(next);
                    }
                }, createIcon('delete'))
            ])
...
function initCleanIPManager() {
    if (rzCleanIPManagerInitialized) return;
    const addBtn = document.getElementById('addCleanIPBtn');
    const input = document.getElementById('cleanIPInput');
    const bulkToggleBtn = document.getElementById('toggleBulkCleanIPBtn');
    const bulkSection = document.getElementById('cleanIPBulkSection');
    const bulkInput = document.getElementById('cleanIPBulkInput');
    const bulkAppendBtn = document.getElementById('bulkAppendBtn');
    const bulkReplaceBtn = document.getElementById('bulkReplaceBtn');
    const bulkCancelBtn = document.getElementById('bulkCancelBtn');
    const errorBox = document.getElementById('cleanIPError');
    const cleanIPsField = document.getElementById('cleanIPs');

    if (!addBtn || !input) return;
    rzCleanIPManagerInitialized = true;
    ...
    const handleAdd = () => {
        ...
        if (!value) return;
        if (!isValidCleanIPHost(value)) {
            showError(t('Invalid IP address or domain'));
            return;
        }
        const current = getCleanIPsList();
        if (current.length >= 40) {
            showError(t('Maximum 40 clean IPs allowed'));
            return;
        }
        if (current.includes(value)) {
            showError(t('Duplicate address ignored'));
            return;
        }
        setCleanIPsList([...current, value]);
        input.value = '';
    };
    ...
    if (bulkToggleBtn && bulkSection) {
        bulkToggleBtn.onclick = () => {
            ...
            if (!bulkSection.hidden && bulkInput) {
                bulkInput.focus();
            }
        };
    }

    if (bulkCancelBtn && bulkSection) {
    ...
    if (bulkAppendBtn) {
        bulkAppendBtn.onclick = () => {
            hideError();
            const incoming = parseBulkAddresses();
            if (incoming.length === 0) return;
            const invalids = incoming.filter(a => !isValidCleanIPHost(a));
            if (invalids.length > 0) {
                showError(`${t('Invalid IP address or domain')}: ${invalids[0]}`);
                return;
            }
            const current = getCleanIPsList();
    ...
    if (bulkReplaceBtn) {
        bulkReplaceBtn.onclick = () => {
            ...
            const deduped = [...new Set(incoming)];
            if (deduped.length > 40) {
                showError(t('Maximum 40 clean IPs allowed'));
                return;
            }
            setCleanIPsList(deduped);
            if (bulkSection) bulkSection.hidden = true;
            if (bulkInput) bulkInput.value = '';
        };
    }
    ...
    if (cleanIPsField) {
        cleanIPsField.addEventListener('input', () => {
            renderCleanIPList(getCleanIPsList());
        });
    }
}
