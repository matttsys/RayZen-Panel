...
export interface Profile {
    ...
    /**
     * Fetches this profile is allowed before it stops working, or null for no limit.
     *
     * Opt-in, because enforcing it changes how often the counter is written: see
     * `shouldPersistUse`.
     */
...
/** Highest request limit that may be set, so the counter and the copy stay sane. */
export const MAX_REQUEST_LIMIT = 1_000_000;

export type ProfileStatus = 'active' | 'expired' | 'disabled' | 'exhausted';
...
export function profileStatus(profile: Profile, now: number): ProfileStatus {
    ...
    if (profile.requestLimit !== null && profile.requests >= profile.requestLimit) return 'exhausted';
    return 'active';
}

/** Requests still allowed, or null when the profile has no limit. */
export function remainingRequests(profile: Profile): number | null {
    if (profile.requestLimit === null) return null;
    return Math.max(0, profile.requestLimit - profile.requests);
}

/** Bounds an operator-supplied request limit. Zero or absent means no limit. */
export function requestLimitFrom(value: unknown): number | null {
    if (value === null) return null;
    const parsed = Number(value);
    if (!Number.isFinite(parsed) || parsed <= 0) return null;
    return Math.min(Math.floor(parsed), MAX_REQUEST_LIMIT);
}
...
export function createProfile(name: unknown, days: unknown, now: number, limit: unknown = null): Profile {
    return {
        token: generateToken(),
        ...
        enabled: true,
        requests: 0,
        requestLimit: requestLimitFrom(limit),
        lastSeenAt: null,
        lastSeenFrom: null
    };
}
...
export function normaliseProfiles(value: unknown): Profile[] {
    ...
    for (const entry of value.slice(0, MAX_PROFILES)) {
        ...
        out.push({
            ...
            requestLimit: requestLimitFrom(row.requestLimit),
...
export function resolveProfile(
    ...
    token: string,
    now: number
): { profile: Profile | null; reason: 'ok' | 'unknown' | 'expired' | 'disabled' | 'exhausted' } {
    ...
    if (status === 'exhausted') return { profile: null, reason: 'exhausted' };
...
export function shouldPersistUse(profile: Profile, now: number): boolean {
    // A limit only means something if the count behind it is exact, so a limited profile
    // is written on every fetch. That is the cost of the feature and it is why the limit
    // is opt-in: an unlimited profile keeps the hourly throttle. A limited profile is by
    // definition meant to be low-volume, so the write budget is not at risk from one.
    if (profile.requestLimit !== null) return true;
