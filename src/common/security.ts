 ...
 *
 * `login`, `proxy-ip` and `error` reach nothing off-origin. `panel` reaches two
 * origins, each recorded with a reason in `tests/golden/icon-subset.test.ts`
 * `ORIGIN_BUDGET`:
 *
 *  - `ipv4.geojs.io` and `ipv4.icanhazip.com`, the user-initiated "My IP" lookup
 *
