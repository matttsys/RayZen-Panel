...
export interface Noise {
    rand?: string;
    randRange?: string;
    type?: 'rand' | 'array' | 'str' | 'base64' | 'hex';
...
export interface FreedomSettings {
    domainStrategy?: DomainStrategy;
    /** Legacy fragmentation, understood by every core from 1.8 up. See cores/xray/compat.ts. */
    fragment?: {
        packets: FragmentPacket;
        length: string;
        interval: string;
    };
    /** Legacy UDP noise, the pre-`finalmask` spelling. */
    noises?: Noise[];
}
...
export interface Config {
    remarks: string;
    /** Absent in `universal` compatibility: a floor makes older clients refuse the profile. */
    version?: {
        min: string;
        max?: string;
    };
