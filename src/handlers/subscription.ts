...
async function resolveProfileRequest(
    ...
    try {
        ...
        // not spend the deployment's KV write budget — except where a request limit is set,
        // which is written on every fetch because a limit enforced against a throttled
        // count would not be a limit. See src/features/profiles.ts.
