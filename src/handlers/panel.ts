...
export async function handlePanel(request: Request, env: Env): Promise<Response> {
    ...
    switch (path) {
        ...
        case 'panel/metrics':
            return getTrafficMetrics(request, env);
...
function getPanelVersion(): Response {
    return respond(true, HttpStatus.OK, undefined, {
        ...
        capabilities: {
            ...
            scanner: true,
            scannerApply: true,
            /** Arbitrary-length apply with per-entry rejection reasons. */
            scannerApplyBatch: true,
            /** `scanner/candidates?range=<cidr>` restricted to one published prefix. */
            scannerCustomRange: true,
            subscriptionUrls: true,
            presets: true,
            cleanIps: true,
            /** `clean-ips` accepts an ordered replace and a delete. */
 ...
 *
 * Lives on `panel/metrics` rather than under `panel/platform/` because it is not a
 * platform capability: it is read straight from the traffic accumulator's KV
 * document and needs neither the capability snapshot nor the request-scoped
 * platform, both of which cost extra KV reads. `panel/platform/metrics` remains the
 * daily counter series.
 */
async function getTrafficMetrics(request: Request, env: Env): Promise<Response> {
    const auth = await authenticate(request, env);
    if (!auth) {
    ...
    if (request.method === 'DELETE') {
        await clearTraffic(env.kv);
        return respond(true, HttpStatus.OK, undefined, { cleared: true }, { 'Cache-Control': 'no-store' });
    }
    ...
    const snapshot = await trafficSnapshot(env.kv);
    return respond(true, HttpStatus.OK, undefined, snapshot, { 'Cache-Control': 'no-store' });
}
...
async function getMyIP(request: Request, env: Env): Promise<Response> {
    ...
    try {
        // An empty body asks about the visitor: Cloudflare already attached the
        // connecting address and its geo properties to this request, so no lookup is
        // needed. A specific address goes through the operator's own endpoint if they
        ...
        if (!ip) {
            ...
            const text = (key: string) => typeof cf[key] === 'string' && cf[key] ? cf[key] as string : undefined;
            if (visitor) {
                return respond(true, HttpStatus.OK, '', {
                    ip: visitor,
