...
/**
 * The version the UI's update check polls (`script.js` `checkVersion`).
 *
 * There is no RayZen release feed yet, so this returns the running build's own
 * version: the check is honest (the running panel is the newest thing that
 * exists), the Update button stays disabled, and the request resolves instead of
 * falling through to the fallback proxy. When a release feed exists, this route
 * is where it plugs in — nothing on the frontend changes.
 */
function getPanelVersion(): Response {
    return respond(true, HttpStatus.OK, undefined, { version: VERSION }, {
