...
import { applyClientCompat } from './compat';
...
async function buildConfig(
    ...
    // The builders above emit the newest Xray shape. Translated here, once, so every
    // config in every subscription goes through the same compatibility decision.
    return applyClientCompat(config);
}
