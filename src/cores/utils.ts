...
import type { DohResponse } from '#types/cloudflare';
...
export async function resolveDNS(domain: string, onlyIPv4 = false): Promise<DnsResult> {
    // Injected in tests so config generation is hermetic and byte-stable.
    // Production leaves this null and takes the DoH path below unchanged.
    const override = runtime.resolveDNS();
    if (override) return override(domain, onlyIPv4);
...
export function generateRemark(
    ...
    // Config naming format is configurable. `remarkSeparator` swaps the field divider
    // (default '•') and `remarkSuffix` sets the trailing brand mark (default '💮',
    // empty string disables it). Defaults produce e.g. "4•Trojan•Clean IP : 443 💮",
    // replacing the old legacy upstream "💮 4. Trojan - Clean IP : 443" style.
    const opt = settings as unknown as { remarkSeparator?: string; remarkSuffix?: string };
...
export function randomUpperCase(str: string): string {
    ...
    for (let i = 0; i < str.length; i++) {
        result += runtime.random() < 0.5 ? str[i].toUpperCase() : str[i];
    }
...
export function getRandomString(lengthMin: number, lengthMax: number): string {
    ...
    const length = Math.floor(runtime.random() * (lengthMax - lengthMin + 1)) + lengthMin;

    for (let i = 0; i < length; i++) {
        result += charSet.charAt(Math.floor(runtime.random() * charSet.length));
    }
...
// These two helpers extend built-in prototypes. They are declared with
// `Object.defineProperty` rather than plain assignment so that they are
// NON-ENUMERABLE, matching how every real built-in method is defined.
//
// Plain assignment (`Object.prototype.omitEmpty = ...`) makes the property
// enumerable, which leaks it into every `for...in` loop over every object in
// the isolate, including objects inside third-party code. Behaviour for all
// existing call sites is identical; only the property descriptor changes.
Object.defineProperty(Array.prototype, 'concatIf', {
    value: function <T>(this: T[], condition: boolean, concat: T | T[]): T[] {
...
Object.defineProperty(Object.prototype, 'omitEmpty', {
    value: function <T>(this: object): T | undefined {
