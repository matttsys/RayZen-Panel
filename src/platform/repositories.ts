...
function createDocument<T>(kv: KVNamespace, key: PlatformKvKey, empty: () => T): Document<T> {
    ...
    const read = async (): Promise<T> => {
        ...
        try {
            ...
            // Merged over the empty shape: the traffic counter writes `rz:metrics` with
            // hourly buckets only, and a document missing `days` used to 500 every
            // route that touched daily counters.
            cached = stored && typeof stored === 'object' && !Array.isArray(stored)
                ? { ...empty(), ...(stored as Partial<T>) }
                : empty();
        } catch {
            cached = empty();
        }
