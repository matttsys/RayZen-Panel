...
export async function TrOverWSHandler(request: Request): Promise<Response> {
    ...
    // Typed locally rather than imported: the data plane takes on no dependencies.
    const remoteSocketWapper: { value: Socket | null } = { value: null };

    const writableStream = new WritableStream({
        async write(chunk, _controller) {
            // No UDP branch here, deliberately. parseTrHeader rejects every
            // command except CONNECT (cmd !== 1), so a UDP stream can never be
            // established on the Trojan path. The VLESS handler has a real
            // udpStreamWrite for its port-53 DoH bridge; Trojan never did. The
            // dead guard that used to sit here was copied from VLESS and was
            // provably unreachable: the variable was declared null and never
