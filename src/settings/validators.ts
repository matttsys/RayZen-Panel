 ...
 *
 * Why each validator is wrapped
 *
 * Every validator reads its fields directly off `form`, which is correct for the
 * complete `PanelSettings` the panel submits and throws for anything less: `{}`
 * fails on `.filter`, and a body missing `antiSanctionDNS` fails on `.match`. The
 * caller catches everything and returns a 500 (`src/handlers/panel.ts:236-239`), so
 * a malformed request was answered as a server fault rather than as the client error
 * it is, and its rejection was never counted.
 *
 * A missing or unparseable field *is* a validation failure, so it is reported as
 * one. Wrapping here rather than adding presence checks to 25 validators keeps their
 * bodies, and their 110 tests, untouched: for a complete form nothing throws and the
 * output is byte-identical to before.
 *
 * This is a backstop, not a licence. A validator that can fail on ordinary input
 * should still return early itself, so the user gets a field-specific message rather
 * than the generic one below. `validateChainProxy` was fixed that way rather than
 * left to this wrapper.
 *
 * The label is the literal `'Settings'` so that
 * `src/features/validation/service.ts` can map it to a stable code; the failing
 * validator's name goes in the message, where it helps a bug report without
 * becoming part of the label contract.
 */
export function validateSettings(form: PanelSettings | null): ValidationError[] | null {
    if (!form || typeof form !== 'object' || Array.isArray(form)) {
        return [{ field: 'Settings', message: ['The settings payload must be a JSON object.'] }];
    }
        ...
        try {
            ...
            errors.push({
                ...
                message: [
                    ...
                    `The check that failed was '${validate.name}'.`
                ]
            });
        }
    });
...
/**
 * The panel's host rule, exported for the Telegram commands.
 *
 * A settings write from a message has to pass the same validation as the form, or the bot
 * becomes the way to store a value the panel would have rejected. `allowPort` is true for
 * proxy addresses, which may carry `:port`, and false for CDN addresses, which may not.
 */
export function isValidProxyHost(value: string, allowPort = true): boolean {
    return allowPort
        ? isValidHost(value, false) || isValidHost(value, true)
        : isValidHost(value, false);
}
...
function validateChainProxy(form: PanelSettings, errors: ValidationError[]) {
    ...
    if (!isVMess && !isOthers) {
        ...
        // Neither regex matched, so `new URL(chainProxy)` below would throw and turn
        // a reportable field error into a 500. The error above is the complete and
        // correct answer for this input; everything after this point inspects a URL
        // that is already known not to be one.
        return;
    }
...
function validateRemoteSettings(form: PanelSettings, errors: ValidationError[]) {
    ...
    if (path !== 'sub/share-settings') {
        errors.push({
            field: 'Remote Settings URL',
            message: ['This is not a valid RayZen remote settings URL']
        });
    }
}
