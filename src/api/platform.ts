...
import { getGlobals, getKvSettings, getWarpAccounts} from '@settings';
...
import { createProfile, MAX_PROFILES, profileStatus, type Profile } from '@features/profiles';
...
const MAX_SCANNER_APPLY_BYTES = 4 * 1024;
const MAX_CLEAN_IPS = 40;
...
async function scannerApply(request: Request, env: Env, platform: Platform): Promise<Response> {
    ...
    const body = value && typeof value === 'object' && !Array.isArray(value)
        ? value as { address?: unknown; mode?: unknown }
        : null;
    if (!body) {
    ...
    if (!address || !isValidProxyHost(address, false)) {
        return respond(false, HttpStatus.BAD_REQUEST, 'address must be a valid IP address or hostname without a port.');
    }
    ...
    const current = Array.isArray(settings.cleanIPs) ? settings.cleanIPs : [];
    const cleanIPs = mode === 'append'
        ? [...new Set([...current, address])]
        : [address];

    if (cleanIPs.length > MAX_CLEAN_IPS) {
        return respond(false, HttpStatus.BAD_REQUEST, `A maximum of ${MAX_CLEAN_IPS} clean IPs is supported.`);
    }
    ...
    return respond(true, HttpStatus.OK, undefined, {
        cleanIPs,
        changed: changed.includes('cleanIPs')
    }, noStore());
}
...
async function linkCreate(request: Request, platform: Platform): Promise<Response> {
    ...
    const body = (await readJson(request)) as { name?: unknown; days?: unknown } | null;
    ...
    const profile = createProfile(body?.name, body?.days, runtime.now().getTime());
...
/**
 * Enables, disables or deletes one link.
 *
...
async function linkUpdate(request: Request, platform: Platform): Promise<Response> {
    ...
    const body = (await readJson(request)) as { token?: unknown; action?: unknown } | null;
    const token = typeof body?.token === 'string' ? body.token : '';
    const action = body?.action;

    if (action !== 'enable' && action !== 'disable' && action !== 'delete') {
        return respond(false, HttpStatus.BAD_REQUEST, "action must be 'enable', 'disable' or 'delete'.");
    }
    ...
    return respond(true, HttpStatus.OK, undefined, { token, action, remaining: next.length }, noStore());
}
