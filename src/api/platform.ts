 ...
 *   - No route accepts a settings write. Presets return a *preview*; the caller
 *     submits the result through the existing `panel/update-settings`, so the
 *     validators stay on the only write path.
...
export const PLATFORM_ROUTES = [
    ...
    'scanner/run',
    'scanner/candidates',
    // Every one of these is a read or a preview: none of them writes
    // settings. Writes continue to go through `panel/update-settings` so the
    // validators keep their monopoly on the write path.
    'health/center',
