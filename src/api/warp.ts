...
export async function fetchWarpAccounts(env: Env): Promise<WarpAccount[]> {
    ...
    try {
        ...
        await createStorage(env.kv).writeWarpAccounts(warpAccounts);
        return warpAccounts;
        ...
        // Persist an explicit empty state so normal requests do not hammer the
        // registration service. The operator can retry from the WARP refresh action.
        // No shared or committed private-key fallback is ever used.
        try {
            await createStorage(env.kv).writeWarpAccounts([]);
        } catch {
            // KV failure is reported by the caller; never replace it with credentials.
        }
